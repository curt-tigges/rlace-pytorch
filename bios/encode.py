...
def encode(bert, texts):
    """This function encodes a list of texts using a BERT model, including masking and tokenization.
    After encoding, the texts are provided to the BERT model as input, and the output is collected and returned.
    ...
    Returns:
        numpy.ndarray: A numpy array of shape (len(texts), bert_hidden_size) containing the BERT embeddings for each text.
    """
    all_H = []
    bert.eval()
    with torch.no_grad():
...
if __name__ == "__main__":
    """This script encodes the bios data using BERT and saves the encodings to disk.
    It also loads the BERT parameters from the finetuned models and encodes the data using those parameters.
    """
    random.seed(0)
    np.random.seed(0)
