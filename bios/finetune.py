...
def load_bios(group):
    ...
    with open("bios_data/{}.pickle".format(group), "rb") as f:
        ...
        random.seed(0)
        np.random.seed(0)
        X, Y, professions, txts, bios_data = sklearn.utils.shuffle(
            X, Y, professions, txts, bios_data
        )
        X = X[:]
        Y = Y[:]
...
adv = args.adv == 1
device = (
    "cuda:{}".format(args.run_id % 4)
    if args.device == -1
    else "cuda:{}".format(args.device)
)
...
X_dev, y_dev_gender, txts_dev, professions_dev = (
    X_dev[:],
    y_dev_gender[:],
    txts_dev[:],
    professions_dev[:],
...
if args.opt == "sgd":
    if not adv:
        ...
        optimizer = torch.optim.SGD(
            list(bert.parameters()) + list(W.parameters(())),
            lr=lr,
            momentum=momentum,
            weight_decay=decay,
        )
        ...
        optimizer = torch.optim.SGD(
            list(bert.parameters()) + list(W.parameters()) + list(adv_clf.parameters()),
            lr=lr,
            momentum=momentum,
            weight_decay=decay,
        )
    ...
    if not adv:
        optimizer = torch.optim.Adam(
            list(bert.parameters()) + list(W.parameters()), lr=1e-4
        )
    else:
        optimizer = torch.optim.Adam(
            list(bert.parameters()) + list(W.parameters()) + list(adv_clf.parameters()),
...
def eval_dev(bert, W, texts_dev, y_dev, device, adv=None, y_dev_gender=None):
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(texts_dev) - batch_size, batch_size):
            ...
            acc = (
                (preds == torch.tensor(batch_y).to(device))
                .float()
                .mean()
                .detach()
                .cpu()
            ...
            if adv:
                ...
                acc = (
                    (preds_adv == torch.tensor(batch_y_gender).to(device))
                    .float()
                    .mean()
                    .detach()
                    .cpu()
...
run = wandb.init(
    reinit=True,
    project="rlace-finetune-bios",
    config=new_d,
    tags=["no-adv" if args.adv == 0 else "mlp-adv" if args.mlp_adv else "linear-adv"],
)
...
for i in pbar:
    ...
    if i % 2000 == 0:  # and i > 0:
        ...
        if dev_loss < best_score:
            ...
            if adv:
                torch.save(
                    adv_clf.state_dict(), "{}/adv_{}.pt".format(path, args.run_id)
                )
